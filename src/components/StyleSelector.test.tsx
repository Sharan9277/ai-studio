...
describe('StyleSelector', () => {
  ...
  test('renders style selector with all options', () => {
    ...
    expect(
      screen.getByRole('option', { name: 'Streetwear' })
    ).toBeInTheDocument();
    expect(screen.getByRole('option', { name: 'Vintage' })).toBeInTheDocument();
    expect(
      screen.getByRole('option', { name: 'Minimalist' })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('option', { name: 'Cyberpunk' })
    ).toBeInTheDocument();
  });
  ...
  test('shows description for selected style', () => {
    ...
    expect(
      screen.getByText('Clean, professional magazine-style aesthetic')
    ).toBeInTheDocument();
  });

  test('can be disabled', () => {
    render(
      <StyleSelector value="editorial" onChange={mockOnChange} disabled />
    );
  ...
  test('updates description when style changes', () => {
    const { rerender } = render(
      <StyleSelector value="editorial" onChange={mockOnChange} />
    );
    ...
    expect(
      screen.getByText('Futuristic, neon-lit high-tech aesthetic')
    ).toBeInTheDocument();
  });
});
